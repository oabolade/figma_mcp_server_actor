...
class FigmaClient {
    ...
    async _request(endpoint, options = {}) {
        ...
        if (this.cacheEnabled && options.method === 'GET' && this.cache.has(cacheKey)) {
    ...
    async getFile(fileKey, options = {}) {
        ...
        if (options.geometry !== undefined) params.append('geometry', options.geometry);
    ...
    async getProjectFiles(projectId, options = {}) {
        ...
        const query = params.toString();
        return this._request(`/projects/${projectId}/files${query ? `?${query}` : ''}`);
    }
