...
export function registerDesignModificationTools(protocol, figmaClient) {
  ...
  protocol.registerTool("update_node_properties", {
    description:
      "Update properties of a design node (Note: Figma REST API has limited write capabilities. Full modification may require Plugin API)",
    inputSchema: {
      type: "object",
      properties: {
        fileKey: {
      ...
      try {
        ...
        return {
          ...
          currentNode: node
            ...
            : null,
          message:
            "Figma REST API has limited write capabilities. For full design modification, please use:",
          options: [
        ...
        throw new Error(
          `Failed to retrieve node information: ${error.message}`,
        );
      }
    },
  });
  ...
  protocol.registerTool("get_node_for_modification", {
    description:
      "Get detailed node information to plan modifications (useful before using Plugin API)",
    inputSchema: {
      type: "object",
      properties: {
        fileKey: {
    ...
    handler: async (args) => {
      ...
      return {
        ...
        modificationGuidance: {
          ...
          recommendedApproach:
            "Use Figma Plugin API for programmatic modifications",
